...
function rotateFour(a) {
    ...
    for (let i = 1; i <= 4; i++) {
        var s = rel[i * 2 - 1];
        for (let j = 0; j < 3; j++) {
            if (s == LEFT) queue.push(`[0][${j}]`);
            if (s == RIGHT) queue.push(`[2][${j}]`);
            if (s == UP) queue.push(`[${j}][0]`);
            if (s == DOWN) queue.push(`[${j}][2]`);
        }
    }
...
function displayKeyDown(key) {
    document.getElementById("key").setAttribute("class", "no-select show");
    document.querySelector("#key .text").innerHTML = key;
    setTimeout(() => {
        document.getElementById("key").setAttribute("class", "no-select hide");
    }, 500);
}

document.onkeydown = e => {
    if (typeof e.key == "string" && checkCommand(e.key)) {
        displayKeyDown(e.key);
        setTimeout(() => {
            launch(e.key);
        }, 100);
    }
